...
| [**`cssforge-core`**](https://crates.io/crates/cssforge-core) | **Pure Rust Library** | [![crates.io](https://img.shields.io/crates/v/cssforge-core.svg?style=flat-square)](https://crates.io/crates/cssforge-core) | Headless AST parser, specificity calculator, and 28 transformation rules with zero UI dependencies. |
...
tar -xzf cssforge-v0.7.0-linux-x64.tar.gz
cp cssforge-v0.7.0-linux-x64/cssforge ~/.local/bin/
chmod +x ~/.local/bin/cssforge
```

#### 🍏 macOS (Apple Silicon M-Series)
```bash
tar -xzf cssforge-v0.7.0-macos-arm64.tar.gz
cp cssforge-v0.7.0-macos-arm64/cssforge ~/.local/bin/
chmod +x ~/.local/bin/cssforge
```

#### 🪟 Windows (x64 / ARM64 Snapdragon)
Extract `cssforge.exe` from `cssforge-v0.7.0-windows-x64.zip` and move it to any directory in your system `Path` (e.g. `C:\Windows\System32` or your tools folder).
...
# List all 28 transformation rules
cssforge rules
```

---

## 🛠️ 28 Transformation Rules Summary
...
* **0.7.0**: `modernize-is` factors `a:hover, button:hover` into `:is(a, button):hover`. Mixed-spec `:is()` without an ID is applied on refactor — **review the diff**; `:is()` takes the highest argument specificity. Identical-body merges (`::selection, .button--primary`) are comma lists and do not change per-selector spec. ID-bearing `:is()` and `:where()` stay REVIEW.
